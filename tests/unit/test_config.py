...
def test_get_prefer_orjson(value, expected_result, monkeypatch):
    monkeypatch.setenv("GLOBUS_SDK_PREFER_ORJSON", value)
    if expected_result is not ValueError:
        assert globus_sdk.config.get_prefer_orjson() == expected_result

    else:
        with pytest.raises(expected_result):
            globus_sdk.config.get_prefer_orjson()
